...
import { generateStrategy, LegalStrategy } from '../services/strategyGenerator';

type RouteP = RouteProp<RootStackParamList, 'Strategy'>;
type NavP = NativeStackNavigationProp<RootStackParamList, 'Strategy'>;
...
const StrategyScreen: React.FC = () => {
  ...
  const [strategy, setStrategy] = useState<LegalStrategy | null>(null);
  const [activeTab, setActiveTab] = useState<'overview' | 'arguments' | 'questions'>('overview');
        ...
        {strategy && (
          <>
            ...
            <View style={styles.tabBar}>
              {(['overview', 'arguments', 'questions'] as const).map(tab => (
                <TouchableOpacity
                  key={tab}
